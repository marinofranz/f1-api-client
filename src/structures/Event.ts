...
export type EventFormat = "conventional" | "sprint" | "sprint_shootout" | "testing"
...
export class Event extends Base<RawScheduleItem> {
  ...
  constructor(data: RawScheduleItem, parent: Schedule) {
    ...
    for (const key in data){
      const value = data[key as keyof RawScheduleItem]

      if (key.startsWith("Session")){
        if (value === "None" || value === "NaT") return;
        if (key.endsWith("Utc")) return;

        const sessionNumber = parseInt(key.charAt(7))

        if (currentSession.number && currentSession.name && currentSession.date) {
  ...
  get isSprint(): boolean {
    return this.eventFormat === "sprint" || this.eventFormat === "sprint_shootout"
  }
  ...
  practice(practiceNumber: number): Session | null {
    if (practiceNumber > 3) throw new Error("Practice sessions are no more than 3.");

    const foundSession = this.sessions.find(session => session.name.startsWith("Practice") && session.number === practiceNumber)
  ...
  session(sessionNumber: number): Session | null {
    if (sessionNumber > 5) throw new Error("Sessions are no more than 5.")

    const foundSession = this.sessions.find(session => session.number === sessionNumber)
    return foundSession || null
  }
}
